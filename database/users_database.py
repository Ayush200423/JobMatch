...
class UsersDatabase:
    ...
    def update_user(self, email, password = None, ents_dict = None, selected_jobs = None):
        if not ents_dict and not selected_jobs:
            self.db_cursor.execute(f"UPDATE users SET password = '{password}' WHERE email = '{email}'")
            self.commit()
        else:
            if ents_dict:
                for kw, value in ents_dict.items():
                    try:
                        self.db_cursor.execute(f"UPDATE users SET {kw} = '{value}' WHERE email = '{email}'")
                        self.commit()
                    except:
                        continue
            elif selected_jobs:
